function varargout = optimize_linesearch_reg(pie, y, M, B, Z, pPDD)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% 2022/11/10
%%%% IMPLEMENT FAST NEWTON-RAPHSON OPTIMIZATION WITH LINE SEARCH, MOMENTUM
%%%% UPDATE AND *REGULARIZATION*
...
objective_all = zeros(3, pPDD.ConvergeMaxIter); % ALLOCATE SPACE FOR OBJECTIVE
...
for iter = 1 : pPDD.ConvergeMaxIter
    ...
    % COMPUTE REGULARIZATION GRADIENT AND HESSIAN
    [garb, hess, obj] = matematik.get_huber(pie, ...
                                            pPDD.HuberDelta, ...
                                            pPDD.HuberBeta, ...
                                            pPDD.HuberImageSize, ...
                                            true, ...
    ...
    % COMPUTE TOTAL GRADIENT AND HESSIAN
    garbage = garbage + garb;
    hessian = hessian + hess;
    objective = objective + obj;
    objective_all(:, iter) = [objective; objective - obj; obj]; % SAVE OBJECTIVE FOR CONVERGENCE CHECKING
    
    % LINE SEARCH (ONE STEPLENGTH FOR ALL PIXELS)
    alphas = numerisk.eval_backtracking_reg(pie, ...
                                            y, ...
                                            M, ...
                                            B, ...
                                            M1, ...
    ...
    % CHECKING TOLERANCE
    if numerisk.eval_tolerance(iter, objective_all(1, :), pPDD.ConvergeTolerance, pPDD.ConvergeRange); break; 
    end
    
    % ADJUST PENALTY
    if logical(pPDD.UseHuberAdjust)
        pPDD.HuberBeta = numerisk.eval_dynamicbeta(iter, ...
                                                   pPDD.HuberBeta, ...
                                                   objective_all(3, :), ...
                                                   objective_all(1, :), ...
                                                   pPDD.HuberAdjustRatio, ...
                                                   pPDD.HuberAdjustRange, ...
                                                   pPDD.HuberAdjustFreq);
    end
    ...
    if logical(pPDD.DrawConvergence)
        if rem(iter, pPDD.DrawUpdateIter) == 1
            ...
            legend('TOTAL', 'DATA FIDELITY', 'REGULARIZATION');
