%% OPPD WITH OPTIMAL LINE SEARCH AND MOMENTUM
clearvars
clc
close all
...
pFNS.OPDD1              = 'demo/opdd_unreg_water_radiograph_customize.mat'; % FOR SAVING, 1ST MATERIAL RADIOGRAPH 
pFNS.OPDD2              = 'demo/opdd_unreg_bone_radiograph_customize.mat'; % FOR SAVING, 2ND MATERIAL RADIOGRAPH 
...
pPDD.UseHuber           = false; % IF USING REGULARIZATION (HERE, NOT)
pPDD.UseHuberAdjust     = false; % IF USING DYNAMIC REGULARIZATION STRENGTH ADJUSTMENT (HERE, NOT)
...
pPDD.HuberImageSize     = []; % GARBAGE HERE, BECAUSE NOT USING REGULARIZATION
pPDD.HuberBeta          = []; % GARBAGE HERE, BECAUSE NOT USING REGULARIZATION
pPDD.HuberDelta         = []; % GARBAGE HERE, BECAUSE NOT USING REGULARIZATION
pPDD.HuberAdjustRatio   = []; % GARBAGE HERE, BECAUSE NOT USING REGULARIZATION
pPDD.HuberAdjustFreq    = []; % GARBAGE HERE, BECAUSE NOT USING REGULARIZATION
pPDD.HuberAdjustRange   = []; % GARBAGE HERE, BECAUSE NOT USING REGULARIZATION
...
[pie, objective_all, pPDD] = opdd.optimize_linesearch_unreg(pie, Y, M, B, Z, pPDD);
